...
长期 memory 使用 Markdown：`user.md` 记录用户偏好，`soul.md` 记录 Agent 沟通方式，
workspace 级 `memory.md` 记录当前 workspace 的长期项目事实。会话中输入 `/dream` 会把新的
compact summary 合并进 workspace memory；`/dream --compact` 会先强制 compact 当前 session。
