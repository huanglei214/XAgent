...
- `config.yaml` 保存模型、provider、workspace、权限、trace、tools、cron、channels 等配置。
...
`xagent/cli/gateway.py` 承载 `xagent gateway` 的 channel manager、AgentLoop 和 CronService 长期运行逻辑。
...
Cron 是 Agent 可管理的长期任务对象，不提供 CLI 管理命令。用户通过自然语言让 Agent 调用
一个 `cron` tool，按 `action=list/create/update/delete` 管理 `~/.xagent/cron/tasks.json`。
`CronService` 是第一版唯一对外服务类，负责文件读写、cron 表达式校验、下一次触发时间计算和
gateway 中的长期 `run()` 循环。

CronService 到点后只向 Bus 投递标准 `InboundMessage`，不直接调用 Agent，也不直接调用 channel：
...
InboundMessage(
    ...
    metadata={"cron": True, "task_id": task.id, "triggered_at": "..."},
)
```

定时任务的执行上下文和回复路由是解耦的：Agent 使用 `cron:<task_id>` 独立 session 执行，
最终结果仍按 `target.channel/chat_id` 发回 Lark 或 Weixin。第一版只支持标准 cron 表达式；
gateway 启动时会跳过已经错过的历史触发时间，并计算下一次 `next_run_at`。
...
- 忽略 `StreamKind.DELTA`，只在 `StreamKind.END` 或 `stream is None` 时发送完整回复。
- `message_format: auto` 下，普通文本使用 `text` 消息，Markdown 结构明显的回复使用
  `interactive` 卡片的 `markdown` 元素；`progress` 和 `error` 仍强制使用文本。
- 按 `chat_id` 发送新消息，不回复原消息 thread。
- `metadata["error"]` 为真时同样发送可见错误文本。

### Weixin / 个人微信 Channel

第一版 `WeixinChannel` 是实验性个人微信 channel，参考 nanobot 的个人微信实现，使用
`ilinkai.weixin.qq.com` HTTP long-poll API 和二维码登录。它不是公众号、企业微信或官方微信
开放平台能力。

标准 channel 名是 `weixin`，私聊 session 身份默认是 `weixin:<from_user_id>`。首版只处理文本：

- `xagent channels login weixin` 负责二维码登录并保存 token/state。
- `xagent gateway` 只加载已登录 state，不在长期运行入口里弹二维码。
- `run()` 循环调用 `ilink/bot/getupdates`，维护 `get_updates_buf` 游标。
- `handle_message()` 忽略 bot 自己消息、重复消息、非文本消息和未授权 sender。
- 收到用户消息时缓存 `chat_id -> context_token`，供后续 `send()` 主动发送文本。
- `send()` 忽略 `StreamKind.DELTA`，只在 `StreamKind.END` 或 `stream is None` 时发送完整文本。
- 不支持图片、语音、文件、视频、typing 或原生 streaming progress。

### InboundMessage

`InboundMessage` 表示从外部进入系统的一条用户消息：

- `channel`：消息来源通道，例如 `cli`、`lark`、`weixin`。
...
- 在普通消息进入 Agent 前截断 slash command，例如 `/dream`。
- 调用 `Agent.run()`。
...
- 使用 `xagent/templates/prompts/*.md` 构造 system、runtime context、summary、dream 和空回复重试 prompt。
- 读取长期 Markdown memory，并注入 system prompt 的 `<memory>` 分区。
- 当前日期、时间和时区属于本轮运行时信息，不写入稳定 system prompt；`Agent` 会把短
  runtime context 作为 ephemeral 内容附加到当前 user message，降低动态时间对长
  system/memory 前缀 token cache 的影响。
- 构造 OpenAI-compatible 模型消息。
- 在收到用户文本后写入 `messages.jsonl`。
- 在上下文超过阈值后触发 summary 压缩，并写入 `summary.jsonl`。
...
`messages.jsonl` 保存原始模型可见消息；`summary.jsonl` 保存 compact 派生摘要；
`session_state.json` 保存 compact 游标；`trace.jsonl` 保存完整调试信息。
...
`files.py` / `search.py` / `shell.py` / `web.py` / `interaction.py` / `cron.py` 放具体工具，
...
summary，并写入 `summary.jsonl`。`session_state.json` 记录最新 summary 和已 compact 的
`messages.jsonl` 游标。后续构造上下文时，最新 summary 会作为 system message 进入模型可见消息，
再拼接 compact 游标之后的原始消息。

长期 memory 使用 Markdown 文件，位于用户级 `~/.xagent/memory`：
...
`user.md` 记录用户个人信息和长期偏好，`soul.md` 记录 Agent 沟通方式，workspace `memory.md`
记录当前 workspace 的长期事实和架构决策。用户明确提供的生日、姓名、常用称呼、所在地等稳定个人事实
应进入 `user.md` 的 `个人信息` 分区，不应写入 workspace memory。Agent 构造 system prompt 时注入：
...
会话内输入 `/dream` 会被 AgentLoop 前置截断，不进入 `Agent.run()`，也不写入普通
`messages.jsonl`。`/dream` 只消费当前 session 中尚未处理过的 `summary.jsonl` 记录，
让模型生成 JSON operations，再由程序应用到 workspace `memory.md`、`user.md` 和 `soul.md`。
支持的操作只有 `append`、`update` 和 `delete`，其中 `update/delete` 必须精确匹配旧 memory
中的完整条目。`/dream --compact` 会先强制 compact 当前 session。dream 细节写入
`trace.jsonl`，用户可见输出固定为 `dreaming...` 和 `dream done.`。
...
`im.message.receive_v1`，以及消息发送和消息表情回复权限。Lark channel 收到有效消息后会给
原消息添加 `OnIt` reaction；回复完成后移除 `OnIt` 并添加 `DONE` reaction。reaction 是
best-effort，失败不影响文本回复。

`channels.weixin` 同样默认关闭。启用前建议先配置 `allow_from`，并通过
`xagent channels login weixin` 完成二维码登录；token 和 cursor 默认保存到
`~/.xagent/channels/weixin/account.json`。
...
- Agent 可通过 `cron` tool 管理定时任务，gateway 可运行 CronService。
- `BaseChannel` 生命周期抽象和 ChannelManager。
- 第一版 `lark` 长连接文本 channel。
- 实验性 `weixin` 个人微信 long-poll 文本 channel。
