...
`user.md` 记录用户长期偏好，`soul.md` 记录 Agent 沟通方式，workspace `memory.md`
记录当前 workspace 的长期事实和架构决策。Agent 构造 system prompt 时注入：
...
合并进 workspace `memory.md`；`/dream --compact` 会先强制 compact 当前 session。dream
细节写入 `trace.jsonl`，用户可见输出固定为 `dreaming...` 和 `dream done.`。
